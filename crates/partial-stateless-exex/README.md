...
| `PS_STORAGE_UNDO=delta\|whole` | how an undo frame records a storage trie the block rewrote (default: `delta`, change records only); `whole` stores the entire trie, the pre-2026-09-18 behavior. Recorded as `storage_undo` in run manifests |
| `PS_TRIE_PARALLEL_MIN=<reveal>,<update>` | process-wide floor under the Exact tries' parallel thresholds (default `0,0`, the historical behavior); in the node it reaches the producer's tries too. Recorded as `trie_parallel_min`; a malformed value fails startup |
